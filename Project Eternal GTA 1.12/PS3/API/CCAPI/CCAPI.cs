...
namespace PS3Lib
{
    public class CCAPI
    {
                ...
                if (!string.IsNullOrEmpty(Path))
                {
                    ...
                    if (File.Exists(DllUrl))
                    {
                            ...
                            .Replace("-", "").Equals(CCAPIHASH))
                        {
                            ...
                            {
                                XtraMessageBox.Show("Impossible to load CCAPI.dll version 2.60.", "CCAPI.dll cannot be load", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
                            XtraMessageBox.Show("You're not using the right CCAPI.dll please install the version 2.60.", "CCAPI.dll version incorrect", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                        XtraMessageBox.Show("You need to install CCAPI to use this library.", "CCAPI.dll not found", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    XtraMessageBox.Show("Invalid CCAPI folder, please re-install it.", "CCAPI not installed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
                XtraMessageBox.Show("You need to install CCAPI to use this library.", "CCAPI not installed", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public int SetConsoleID(string consoleID)
        {
            ...
            {
                XtraMessageBox.Show("Cannot send an empty value", "Empty or null console id", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
        ...
        public int SetPSID(string PSID)
        {
            ...
            {
                XtraMessageBox.Show("Cannot send an empty value", "Empty or null psid", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return -1;
            }
        ...
        internal static byte[] StringToByteArray(string hex)
        {
            try
            {
                ...
                XtraMessageBox.Show("Incorrect value (empty)", "StringToByteArray Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return new byte[1];
            }
        }
     }
